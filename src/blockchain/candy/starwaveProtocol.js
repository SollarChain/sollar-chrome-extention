...
class starwaveProtocol {
   ...
   */
  createMessage(data, reciver, sender, id, timestamp = undefined, TTL = undefined, relevancyTime = undefined, route = undefined, type = undefined, timestampOfStart = undefined, broadcastId = '') {
    return {
      data: data,
      reciver: reciver,
    ...
    return this.candy.sockets.map(function (s) {
      if (s && s.readyState === 1) {
        if (fullSockets) {
          ...
          return 'ws://' + s._socket.remoteAddress + ':' + /*s._socket.remotePort*/ this.candy.config.p2pPort
        }
      }
