...
const BlockchainRequestors = {
  ...
  queryBlocksMsg(fromIndex, limit, offset, hideEmptyBlocks) {
    limit = typeof limit === 'undefined' ? 1 : limit;
    return {
...
function Candy(nodeList) {
  ...
  try {
    ...
    this.starwaveProtocol = starwaveProtocol;
    this.NodeMetaInfo = NodeMetaInfo;
    this.DigitalSignature = digitalSignature;
    this.StarwaveCrypto = starwaveCrypto;
  ...
  this._dataRecieved = function (source, data) {
      ...
      try {
        ...
        for (let a in blocks) {
          ...
          if (that.blockHeight < block.index) {
            const newBlocksCount = that.isFetchedAnyBlocks
              ? block.index - that.blockHeight
              : 0;
            blocksStore.fetchNewBLocks(block.index, newBlocksCount).then();

            that.blockHeight = block.index;
            candyEventsStore.onBlockHeightChange(that.blockHeight);
          }
    ...
    if (data.type === MessageType.MY_PEERS) {
      ...
      candyEventsStore.onConnectionsCountChange(that.nodeList.filter(Boolean).length);
    }
   ...
   */
  this.loadBlocks = function (blockId, limit = 1, offset = 0, hideEmptyBlocks = false) {
