...
class StarwaveProtocol {
  ...
  constructor(candy: any, messageType: any) {
    this.candy = candy;
   ...
   */
  createMessage(data: any, reciver: string, sender: string, id: any, timestamp = undefined, TTL = undefined, relevancyTime = undefined, route = undefined, type = undefined, timestampOfStart = undefined, broadcastId = '') {
    return {
      data: data,
      reciver: reciver,
   ...
   * @return {boolean}
   */
  registerMessageHandler(message: any, broadcastId = '', handler: any = null) {
    let that = this;
    ...
    if (typeof that.candy !== 'undefined') {
      this.candy.registerMessageHandler(message, function (messageBody: any) {
        if (
   ...
   */
  sendMessageToPeer(messageBusAddress: string, message: any) {
    let that = this;
    if (typeof that.candy !== 'undefined') {
   ...
   * @returns {Boolean}
   */
  manageIncomingMessage(message: any) {
    ...
    if (this.endpointForMessage(message)) {
      ...
      if (message.route.length > 1) { //if the route consist of the only element we don't save the route becase of direct connection to peer
        ...
        this.candy.routes[message.sender] = message.route.reverse().filter((v: any, i: number, a: any) => a.indexOf(v) === i);
      }
   ...
   * @private
   */
  retranslateMessage(message: any) {
   ...
   */
  handleMessage(message: any, messagesHandlers: any, ws: any) {
   ...
   * @param messageBody
   */
  handleMessageMutex(messageBody: any) {
   ...
   * @returns {boolean}
   */
  endpointForMessage(message: any) {
   ...
   * @returns {Array}
   */
  getCurrentPeers(fullSockets: any) {
    return this.candy.sockets.map((s: any) => {
      if (s && s.readyState === 1) {
        if (fullSockets) {
          return s;
        } else {
          const p2pPort: any = this.candy.config.p2pPort;
          return `ws://${s._socket.remoteAddress}:${p2pPort}`;
        }
      }
   ...
   * @return {*}
   */
  getSocketByBusAddress(address: string) {
   ...
   * @param excludeIp
   */
  broadcast(message: string, excludeIp: string) {
    let i;
   ...
   */
  preventMultipleSockets(socket: any) {
    let busAddress;
...
export default StarwaveProtocol;