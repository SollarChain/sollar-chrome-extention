 ...
 *
 * This module uses elliptic library for creating keypair and blockchain-js to encrypt/decrypt message
...
class StarwaveCrypto {
  elliptic: any = elliptic;
  CryptoJS: any = CryptoJS;
  ec: any;
  keyObject: any;
  public: any;
  starwave: any;
  secretKeys: any;
  curve: any;
  _connectionsCallbacks: any = {};

  constructor(starwaveProtocolObject: any, secretKeysKeyring: any, curve: string = 'secp256k1') {
    // EСDH object
    this.ec = new this.elliptic.ec(curve);
    ...
    starwaveProtocolObject.registerMessageHandler(SWCRYPTO_CONNECTION_MESSAGE_TYPE, (message: any) => {
      this.handleIncomingMessage(message);
      return true;
    });

    starwaveProtocolObject.registerMessageHandler('', (message: any) => {
      this.handleIncomingMessage(message);
      return false;
    });
  };
   ...
   * @returns {*}
   */
  generateKeys(keyObject: any = this.keyObject) {
   ...
   * @returns {string}
   */
  cipherData(data: string, secret: string) {
    let encrypted = this.CryptoJS.AES.encrypt(data, secret).toString(); //base64
    let b64 = this.CryptoJS.enc.Base64.parse(encrypted);//object
    encrypted = b64.toString(this.CryptoJS.enc.Hex);//hex
    return encrypted;
  }
   ...
   * @returns {string}
   */
  decipherData(encryptedData: string, secret: string) {
    let data;
    try {
      //unpack from hex to native base64 and to object
      let b64 = this.CryptoJS.enc.Hex.parse(encryptedData);
      let bytes = b64.toString(this.CryptoJS.enc.Base64);
      data = this.CryptoJS.AES.decrypt(bytes, secret);
      data = data.toString(this.CryptoJS.enc.Utf8);
    } catch (e) {
   ...
   */
  makeConnection(messageBus: any, cb?: any) {
    let that = this;
    if (cb) {
